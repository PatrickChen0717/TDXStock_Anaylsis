...
from tensorflow.keras.layers import Dense, Reshape, Flatten, Input, Dropout, Activation, LSTM, ReLU, GRU
...
def config_logger(module_name, log_level=logging.INFO):
...
logger = config_logger(module_name=__name__)
##########################################################################################
...
def data_plot(full_prices, expected_price):
    plt.figure(figsize=(12, 6))

    # Plot real data (concatenation of input and expected output)
    plt.plot(full_prices, label='Predicted Data', alpha=0.5, linewidth=1)
    plt.plot(expected_price, label='Expected Data', alpha=0.5, linewidth=1)
...
df_price_array = df_price.values

x_train = []
y_train = []

for i in range(timeframe, len(df_price_array)):
    x_train.append(df_price_array[i-timeframe:i, :])
    y_train.append(df_price_array[i, :])

logger.info(f"Shape of df_price_array: {df_price_array.shape}")
logger.info(f'x_train shape: {np.array(x_train).shape}')
logger.info(f'y_train shape: {np.array(y_train).shape}')

x_train, y_train = np.array(x_train), np.array(y_train)

def train_LSTM(x_train, y_train):
    # Reshape the training data to fit the LSTM input shape [samples, time steps, features]
    # x_train = np.reshape(x_train, (x_train.shape[0], x_train.shape[1], 1))

    # LSTM Model
    model = Sequential()
    logger.info((x_train.shape[1], x_train.shape[2]))
...
def test_prediction_rate(models):
    df = pd.read_csv(csv_path)
    df_price = df.iloc[::2, :].reset_index(drop=True)
...
if __name__ == '__main__':
    ...
    LSTM_model.save('ml/GAN/lstm_1000.h5')

    LSTM_model = load_model('ml/GAN/lstm_1000.h5')

    recent_10_days = df_price.tail(timeframe).to_numpy()
    recent_10_days = np.reshape(recent_10_days, (1, timeframe, 240)) 
    print(recent_10_days.shape)

    predicted_price = LSTM_model.predict(recent_10_days)

    df = pd.read_csv('ml/test_data1.csv')
    df_price = df.iloc[::2, :].reset_index(drop=True)
    df_price = df_price * 100

    logger.info(f"predicted_price shape: {np.array(predicted_price).shape}")
    logger.info(f"Expected_price shape: {df_price.shape}")


    data_plot(predicted_price.reshape(-1), df_price.to_numpy().reshape(-1))